...
def lambda_handler(event, context):
    try:
        ...
        sql = sql_builders.revenue_monthly_sql(domain_cfg["table"], domain_cfg["date_column"], where_clause)
        rows = db.query(sql, where_params)

        filters = validation.build_filters_payload(domain, client, start_date, end_date, ascribe_filters)
        log_info("analytics_revenue_monthly_success", domain=domain, row_count=len(rows), filters=filters)
        return success(domain, filters, {"items": rows})
    except validation.ValidationError as exc:
        return bad_request(str(exc))
    except Exception as exc:
        log_error("analytics_revenue_monthly_failed", error=str(exc))
