...
def lambda_handler(event, context):
    try:
        ...
        dimension_column = validation.parse_breakdown_dimension(params, domain)
        top_n = validation.parse_top_n(params)
        ...
        sql = sql_builders.breakdown_sql(
            domain=domain,
        ...
        log_info("analytics_breakdown_success", domain=domain, row_count=len(rows), filters=filters)
        return success(domain, filters, {"dimension": dimension_column, "items": rows})
    except validation.ValidationError as exc:
        return bad_request(str(exc))
    except Exception as exc:
        log_error("analytics_breakdown_failed", error=str(exc))
